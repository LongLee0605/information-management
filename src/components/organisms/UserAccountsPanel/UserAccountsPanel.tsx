...
function AccountTypeBadge({ account }: { account: EnrichedBankAccount }) {
  return (
    <span
      className={cn(
        'inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold',
        ACCOUNT_TYPE_COLORS[account.accountType] ?? 'border-border bg-table-stripe text-foreground-soft',
      )}
    >
...
function StatusBadge({ status }: { status: EnrichedBankAccount['status'] }) {
  if (status === 'active') {
    return (
...
export function UserAccountsPanel({
  ...
  onRowClick,
  emptyMessage = 'Khách hàng chưa có tài khoản nào.',
  enableFilter = true,
  enableDelete = true,
  ...
  const filteredAccounts = useMemo(
    () => (enableFilter ? filterBankAccounts(accounts, appliedFilters) : accounts),
    [accounts, appliedFilters, enableFilter],
  );
  ...
  return (
    <div className="dashboard-card overflow-hidden">
      ...
      <DataTable
        columns={[
            ...
            ? [
                {
                  key: 'customer',
                  header: 'Khách hàng',
                  render: (row: EnrichedBankAccount) => (
            ...
            : []),
          {
            ...
            render: (row) => (
              <span className="font-mono text-xs font-semibold text-primary-700">{row.cif}</span>
            ),
          },
          {
            key: 'type',
          ...
          },
          {
            ...
            className: 'whitespace-nowrap',
            render: (row) => (
              <span className="font-mono text-xs">{formatAccountNumberDisplay(row.accountNumber)}</span>
            ),
          },
          {
            key: 'bank',
          ...
          },
          {
            ...
            className: 'text-right whitespace-nowrap',
            render: (row) => (
              <span className={row.frozenBalance > 0 ? 'text-expense' : 'text-foreground-soft'}>
          ...
          },
          {
            ...
            className: 'text-right whitespace-nowrap',
            render: (row) => (
              <span className="font-medium text-income">{formatCurrency(row.availableBalance)}</span>
            ),
          },
          {
            key: 'status',
            ...
            ? [
                {
                  ...
                  render: (row: EnrichedBankAccount) => (
                    <button
                      ...
                      aria-label={`Xóa tài khoản ${formatAccountNumberDisplay(row.accountNumber)}`}
                      onClick={(event) => {
        ...
        paginate={filteredAccounts.length > 10}
      ...
      {deleteError && (
        <p className="border-t border-border px-6 py-3 text-sm text-red-600">{deleteError}</p>
      )}
    </div>
  );
}
