...
import { getAccountByNumber } from '@/utils/accountRegistry';
import {
  generateTransferId,
  getRecipientNameByAccount,
  getSourceAccounts,
  type TransferDraft,
...
export function TransferFlow({ defaultUserId, returnPath }: TransferFlowProps) {
  ...
  const [sourceAccounts, setSourceAccounts] = useState(getSourceAccounts);
  const defaultAccount =
    sourceAccounts.find((account) => account.userId === defaultUserId)
    ?? sourceAccounts[0];

  const [step, setStep] = useState<number>(TRANSFER_STEPS.FORM);
  const [sourceUserId, setSourceUserId] = useState(defaultAccount?.userId ?? '');
  ...
  useEffect(() => {
    return subscribeTransactionChange(() => {
      setSourceAccounts(getSourceAccounts());
    });
  }, []);
  ...
  const recipientName = getRecipientNameByAccount(recipientAccount);
  ...
  async function handleConfirmOtp() {
    ...
    const recipient = getAccountByNumber(recipientAccount);
    if (!recipient) {
      setFormError('Số tài khoản nhận không hợp lệ hoặc chưa được xác minh.');
      return;
    }
