    ...
    <NavLink
      ...
      className={({ isActive }) =>
        cn(
          ...
          (isActiveMatch ? isActiveMatch(window.location.pathname) : isActive)
...
function DisabledNavItem({ icon, label }: { icon: React.ReactNode; label: string }) {
  return (
    <span
      ...
      title="Chọn khách hàng từ danh sách trước"
    >
      <span className="h-5 w-5 shrink-0">{icon}</span>
      {label}
    </span>
...
export function Sidebar() {
  ...
  const isReportsPage = Boolean(
    userId && location.pathname === userReportsPath(userId),
  );
  ...
  return (
    <aside className="hidden w-64 shrink-0 flex-col bg-sidebar text-sidebar-text md:flex">
      ...
      <nav className="flex-1 space-y-1 px-3 py-4" aria-label="Menu chính">
        ...
        <NavLink
          to={userId ? userTransactionsPath(userId) : ROUTES.TRANSACTIONS}
          className={() =>
            cn(
              'flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors',
              isTransactionsPage
        ...
        <div className="pt-2">
          <button
            type="button"
            onClick={() => setManualReportsOpen((open) => !(open ?? isReportsSection))}
            className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-sidebar-text outline-none transition-colors hover:bg-sidebar-hover"
          ...
          {reportsOpen && (
            <div className="ml-4 mt-1 space-y-1 border-l border-white/10 pl-3">
              {userId ? (
                ...
                <span
                  className="block cursor-not-allowed rounded-lg px-3 py-2 text-sm text-sidebar-muted opacity-60"
                  title="Chọn khách hàng từ danh sách trước"
                >
                  <span className="flex items-center gap-2">
                    <ChartIcon className="h-4 w-4" />
