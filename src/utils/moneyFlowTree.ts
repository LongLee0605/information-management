...
export function flattenMoneyFlowNodes(
  root: MoneyFlowNode,
...
export function filterMoneyFlowTree(
  node: MoneyFlowNode,
  maxLevel: number,
  expandedBranches: ReadonlySet<string>,
): MoneyFlowNode {
  ...
  return {
    ...
    children: expandChildren
      ? (node.children ?? []).map((child) =>
          filterMoneyFlowTree(child, maxLevel, expandedBranches),
        )
      : undefined,
  };
}
...
export function filterFlatMoneyFlowNodes(
  items: FlatMoneyFlowNode[],
  options: {
    query?: string;
    level?: number | 'all';
  },
...
export function findMoneyFlowNode(
  root: MoneyFlowNode,
  nodeId: string,
): FlatMoneyFlowNode | null {
