...
function buildMoneyFlowParams(filters: Partial<MoneyFlowFilterParams> & { customerId?: string }) {
  return {
...
export async function searchMoneyFlow(filters: MoneyFlowFilterParams): Promise<MoneyFlowSearchResult> {
  ...
  const { data } = await api.get<ApiMoneyFlowRow[]>(
    API_PATHS.reports.moneyFlow,
