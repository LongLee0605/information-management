...
export default function AccountPage() {
    ...
    const { accounts, loading: accountsLoading, error: accountsError, refetch: refetchAccounts, } = useCustomerAccounts(id);
    ...
    return (<UserPageShell user={user} loading={loading} notFound={notFound} error={error} title="Quản Lý Tài Khoản" subtitle={user ? `CIF ${accounts[0]?.cif ?? '—'} · ${user.fullName}` : undefined} summary={summary} onRetry={refetchAll} toolbarActions={userId ? (<div className="flex flex-wrap gap-2">
      ...
      {user && (<UserAccountsPanel accounts={accounts} loading={accountsLoading} error={accountsError} onRetry={refetchAccounts}/>)}

      <AddAccountModal open={showAddModal} defaultCif={defaultCif} onClose={() => setShowAddModal(false)} onSuccess={refetchAccounts}/>
    </UserPageShell>);
}
