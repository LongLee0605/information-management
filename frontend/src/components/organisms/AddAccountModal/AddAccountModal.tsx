...
const ACCOUNT_TYPE_CREATE_OPTIONS: {
...
function AccountSuccessView({ account, onBack, }: {
    account: EnrichedBankAccount;
    onBack: () => void;
}) {
    const typeLabel = ACCOUNT_TYPE_CREATE_OPTIONS.find((option) => option.value === account.accountType)?.label ?? account.accountTypeLabel;
...
function AddAccountModalContent({ defaultCif, onClose, onSuccess, }: AddAccountModalContentProps) {
    ...
    async function handleCheckCif() {
        ...
        try {
            ...
            setVerifiedCif(null);
            setError(checkError instanceof Error
                ? checkError.message
    ...
    if (step === 'success' && createdAccount) {
        ...
        <div role="dialog" aria-modal="true" aria-labelledby="add-account-success-title" className="dashboard-card relative w-full max-w-xl overflow-hidden p-6">
    ...
    return (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div role="dialog" aria-modal="true" aria-labelledby="add-account-title" className="dashboard-card w-full max-w-xl p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <div className="flex gap-2">
              <input type="text" value={form.cif} onChange={(event) => updateField('cif', event.target.value.replace(/\D/g, ''))} onBlur={() => {
            if (!verifiedCif && canCheckCif) {
                void handleCheckCif();
            }
        }} className="form-input flex-1" placeholder="Nhập số CIF" inputMode="numeric"/>
              {verifiedCif ? (<span className="inline-flex shrink-0 items-center gap-1.5 rounded-md bg-green-600 px-4 py-2 text-sm font-medium text-white">
                  <CheckIcon className="h-4 w-4"/>
                  Hợp lệ
                </span>) : (<Button variant="secondary" type="button" className="shrink-0" disabled={!canCheckCif || checkingCif} onClick={handleCheckCif}>
                  {checkingCif ? 'Đang kiểm tra...' : 'Kiểm tra'}
                </Button>)}
            </div>
          </div>

          {verifiedCif && (<div className="rounded-xl border border-green-200 bg-green-50 px-4 py-3">
          ...
          <label className="block">
            ...
            <Select value={form.accountType} onChange={(value) => updateField('accountType', value as BankAccountType | '')} options={[
            { value: '', label: 'Chọn loại tài khoản' },
            ...ACCOUNT_TYPE_CREATE_OPTIONS.map((option) => ({
                value: option.value,
                label: option.label,
            })),
        ]} disabled={!verifiedCif} aria-label="Loại tài khoản"/>
          </label>

          {verifiedCif && (<div className="rounded-xl border border-primary-100 bg-primary-50 px-4 py-3 text-sm">
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button variant="green" type="submit" disabled={!isValid || saving} className={cn(!isValid && 'opacity-50')}>
              {saving ? 'Đang lưu...' : 'Lưu'}
            </Button>
          </div>
        </form>
...
export function AddAccountModal({ open, onClose, onSuccess, defaultCif = '', }: AddAccountModalProps) {
    if (!open) {
        return null;
    }
    return (<AddAccountModalContent key={defaultCif || 'manual'} defaultCif={defaultCif} onClose={onClose} onSuccess={onSuccess}/>);
}
