      ...
      {data.map((item, index) => {
        ...
        return (
          <li
            ...
            className={cn(
              'flex min-w-0 cursor-pointer items-center gap-2.5 rounded-md border px-3 py-2 text-sm transition-all duration-200',
              isActive
          ...
          >
            <span
              className={cn(
                'h-2.5 w-2.5 shrink-0 rounded-full ring-2 ring-white/10 transition-transform duration-200',
                isActive && 'scale-125',
              )}
              style={{ backgroundColor: isActive ? enrichColor(item.fill) : item.fill }}
              aria-hidden="true"
            />
            <span
              className={cn(
                'min-w-0 flex-1 truncate transition-colors',
                isActive ? 'font-medium text-foreground' : 'text-foreground-soft',
              )}
            >
              {item.name}
            </span>
            <span
              className={cn(
                'shrink-0 font-bold tabular-nums transition-colors',
...
function SinglePie({ title, data }: SinglePieProps) {
  ...
  const sectorShape = useMemo(
    () => createSectorShape(data, activeIndex),
    [data, activeIndex],
  );
  ...
  if (!data.length) {
    return (
      <div className="dashboard-card rounded-2xl p-5 sm:p-7">
