...
import { RECIPIENT_BANKS, TRANSFER_STEPS, type RecipientBankId, } from '@/constants/transfer';
import { ROUTES, userTransactionsPath } from '@/constants';
import { createTransferTransaction } from '@/services';
import { fetchSourceAccounts, generateTransferId, lookupRecipientByAccount, type SourceAccount, type TransferDraft, } from '@/utils/transferAccounts';
...
function BackButton({ onClick }: {
    ...
    return (<button type="button" onClick={onClick} className="flex h-9 w-9 items-center justify-center rounded-full border border-border bg-white text-foreground-soft outline-none hover:bg-table-stripe" aria-label="Quay lại">
      ←
    </button>);
}
...
export function TransferFlow({ defaultUserId, returnPath }: TransferFlowProps) {
    ...
    useEffect(() => {
        ...
        fetchSourceAccounts().then((accounts) => {
            ...
            if (!accounts.length)
                return;
            const preferred = accounts.find((account) => account.userId === defaultUserId) ?? accounts[0];
            setSourceUserId((current) => accounts.some((account) => account.userId === current) ? current : preferred.userId);
        });
        return () => {
            cancelled = true;
        };
    ...
    const stepTitle = step === TRANSFER_STEPS.FORM
        ? 'Chuyển Tiền'
        : step === TRANSFER_STEPS.OTP
            ? 'Xác Thực OTP'
            : 'Chuyển Tiền Thành Công!';
    const stepSubtitle = step === TRANSFER_STEPS.FORM
    ...
    return (<div className="mx-auto max-w-3xl">
      ...
      {step === TRANSFER_STEPS.FORM && selectedAccount && (<div className="dashboard-card space-y-6 p-6 sm:p-8">
          <SourceAccountSelect accounts={sourceAccounts} value={sourceUserId} onChange={setSourceUserId}/>
          ...
          <div className="space-y-4">
            ...
            <label className="block">
              ...
              <input type="text" inputMode="numeric" value={recipientAccount} placeholder="Nhập số tài khoản người nhận" onChange={(event) => setRecipientAccount(event.target.value.replace(/\D/g, ''))} className={inputClassName}/>
            </label>

            {recipientAccount.trim() && recipientName && (<div className="flex items-center gap-2 rounded-xl bg-green-50 px-4 py-3 text-sm font-medium text-green-700">
                <span aria-hidden="true">✓</span>
                {recipientName}
              </div>)}

            {recipientAccount.trim() && !recipientName && (<p className="text-sm text-amber-600">
            ...
            <label className="block">
              ...
              <input type="text" inputMode="numeric" value={amount} placeholder="Nhập số tiền chuyển" onChange={(event) => setAmount(formatVndInput(event.target.value))} className={inputClassName}/>
            </label>

            <label className="block">
              <span className="mb-2 block text-sm text-muted">Nội dung chuyển tiền</span>
              <input type="text" value={content} placeholder="Nhập nội dung chuyển khoản" onChange={(event) => setContent(event.target.value)} className={inputClassName}/>
            </label>
          </div>
      ...
      {step === TRANSFER_STEPS.OTP && selectedBank && (<div className="dashboard-card p-6 sm:p-8">
          ...
          {formError && (<p className="mt-4 rounded-lg bg-red-50 px-4 py-3 text-sm text-red-600">{formError}</p>)}

          <Button variant="primary" type="button" className="mt-8 w-full py-3" disabled={otp.length !== 6 || submitting} onClick={() => void handleConfirmOtp()}>
      ...
      {step === TRANSFER_STEPS.SUCCESS && draft && createdTransaction && (<>
          ...
          <div className="relative dashboard-card overflow-hidden p-6 sm:p-8">
            ...
            ].map(([label, value]) => (<div key={label} className="flex items-center justify-between gap-4 border-b border-border py-2">
                    <span className="text-sm text-muted">{label}</span>
                    <span className="text-sm font-semibold text-foreground">{value}</span>
                  </div>))}
              </div>

              <Link to={defaultUserId
                ? userTransactionsPath(draft.sourceUserId)
                : ROUTES.TRANSACTIONS} className="mt-8 block">
