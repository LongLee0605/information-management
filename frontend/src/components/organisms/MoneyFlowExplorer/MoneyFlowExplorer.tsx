...
export function MoneyFlowExplorer({ root, className }: MoneyFlowExplorerProps) {
  ...
  const selectedNode = useMemo(
    () => (highlightedNodeId ? findMoneyFlowNode(root, highlightedNodeId)?.node ?? null : null),
    [root, highlightedNodeId],
  );

  const selectedPath = useMemo(
    () => (highlightedNodeId ? getNodePath(root, highlightedNodeId) : []),
    [root, highlightedNodeId],
  );
  ...
  return (
    <div className={cn('dashboard-card overflow-hidden', className)}>
      ...
      {selectedNode && (
        <div className="border-b border-primary-100 bg-primary-50/60 px-4 py-3 sm:px-6">
      ...
      {view === 'diagram' ? (
        <div className="grid items-start gap-0 xl:grid-cols-[minmax(0,1fr)_300px]">
          <MoneyFlowDiagram
            key={`diagram-${rootId}`}
            root={root}
            highlightedNodeId={highlightedNodeId}
            onHighlightNode={setHighlightedNodeId}
            onVisibleRootChange={handleVisibleRootChange}
            embedded
          />
          <MoneyFlowNodeList
            key={`list-${rootId}`}
