...
import { countDescendants, filterMoneyFlowTree, getNodePath, resolveRevealViewDepth, resolveViewDepth, type MoneyFlowViewDepth, } from '@/utils/moneyFlowTree';
...
function getChildrenLayout(children: MoneyFlowNode[], cardWidth: number, gap: number): {
    totalWidth: number;
    layouts: ChildLayout[];
} {
...
function buildConnectorPath(parentCenterX: number, childCenterXs: number[], busY: number, endY: number): string {
...
function TreeConnector({ width, parentCenterX, childCenterXs, }: {
    width: number;
    ...
    return (<div className="shrink-0" style={{ width, paddingTop: CONNECTOR_GAP }}>
      <svg width={width} height={height} className="block overflow-visible" aria-hidden>
        <path d={pathD} fill="none" stroke={CONNECTOR_COLOR} strokeWidth={CONNECTOR_WIDTH} strokeLinecap="round" strokeLinejoin="round"/>
      </svg>
    </div>);
}
...
const FlowNodeCard = memo(function FlowNodeCard({ node, sourceNode, compact, highlighted, selected, hiddenDescendants, onExpandBranch, onSelect, cardRef, }: FlowNodeCardProps) {
    ...
    return (<div ref={cardRef} role="button" tabIndex={0} onClick={() => onSelect?.(node.id)} onKeyDown={(event) => {
      ...
      <div className="flex h-9 shrink-0 items-center justify-center">
        <Avatar src={node.avatar ?? getAvatarUrl(node.id, 'male')} alt={node.fullName} size="sm"/>
      </div>

      <p className={cn('h-8 shrink-0 overflow-hidden px-0.5 text-[11px] font-bold leading-[1.15]', isRoot ? 'text-white' : 'text-primary-700')} style={{
            display: '-webkit-box',
            WebkitLineClamp: 2,
      ...
      <p className={cn('flex h-4 shrink-0 items-center justify-center text-[11px] font-bold leading-none', amountLabel ? 'text-orange-500' : 'invisible')}>
        {amountLabel ?? '—'}
      </p>

      <p className={cn('flex h-3.5 shrink-0 items-center justify-center truncate px-1 text-[8px] leading-none', isRoot ? 'text-blue-100/80' : 'text-slate-400', !node.periodLabel && 'invisible')}>
        {node.periodLabel ?? '—'}
      </p>
      ...
      <div className="mt-auto flex h-5 shrink-0 items-center justify-center">
        {showExpand ? (<button type="button" className="w-full truncate rounded border border-primary-200 bg-primary-50 px-1 py-0.5 text-[8px] font-semibold leading-tight text-primary-700 hover:bg-primary-100" onClick={(event) => {
                event.stopPropagation();
                onExpandBranch?.(sourceNode.id);
            }}>
            +{hiddenDescendants} nhánh
          </button>) : (<span className="invisible text-[8px]">—</span>)}
      </div>
    </div>);
});
function FlowSubtree({ node, sourceNode, compact, highlightedNodeId, hiddenCounts, onExpandBranch, onSelect, registerNodeRef, cardWidth, siblingGap, }: {
    node: MoneyFlowNode;
    sourceNode: MoneyFlowNode;
    compact: boolean;
    highlightedNodeId?: string | null;
    ...
    return (<div className="flex flex-col items-center" style={{ width: columnWidth }}>
      <FlowNodeCard node={node} sourceNode={sourceNode} compact={compact} highlighted={highlightedNodeId === node.id} selected={highlightedNodeId === node.id} hiddenDescendants={hiddenCounts.get(sourceNode.id) ?? 0} onExpandBranch={onExpandBranch} onSelect={onSelect} cardRef={(element) => registerNodeRef(node.id, element)}/>

      {children.length > 0 && (<>
          <TreeConnector width={childLayout.totalWidth} parentCenterX={columnWidth / 2} childCenterXs={childLayout.layouts.map((layout) => layout.centerX)}/>
          <div className="flex items-start" style={{ width: childLayout.totalWidth }}>
            {childLayout.layouts.map((layout, index) => (<div key={layout.node.id} style={{
                    width: layout.width,
                    flexShrink: 0,
                    marginLeft: index > 0 ? siblingGap : 0,
                }}>
                <FlowSubtree node={layout.node} sourceNode={layout.node} compact={compact} highlightedNodeId={highlightedNodeId} hiddenCounts={hiddenCounts} onExpandBranch={onExpandBranch} onSelect={onSelect} registerNodeRef={registerNodeRef} cardWidth={cardWidth} siblingGap={siblingGap}/>
              </div>))}
          </div>
        </>)}
    </div>);
...
export function MoneyFlowDiagram({ root, className, highlightedNodeId, onHighlightNode, onVisibleRootChange, embedded = false, }: MoneyFlowDiagramProps) {
    ...
    const visibleRoot = useMemo(() => filterMoneyFlowTree(root, resolveViewDepth(effectiveViewDepth), effectiveExpandedBranches), [root, effectiveViewDepth, effectiveExpandedBranches]);
    const hiddenCounts = useMemo(() => buildHiddenCounts(visibleRoot, root), [visibleRoot, root]);
    useEffect(() => {
    ...
    const innerWidth = useMemo(() => Math.max(cardWidth, f1Layout.totalWidth), [cardWidth, f1Layout.totalWidth]);
    useLayoutEffect(() => {
    ...
    const fitToView = useCallback(() => {
        ...
        const nextZoom = Math.min(ZOOM_MAX, Math.max(ZOOM_MIN, Math.min(zoomByWidth, zoomByHeight)));
    ...
    return (<div className={shellClass}>
      <div className="panel-header flex flex-wrap items-center justify-between gap-2">
        <div className="flex flex-wrap gap-1">
          {DEPTH_OPTIONS.map((option) => (<button key={option.value} type="button" onClick={() => {
                setViewDepth(option.value);
                setExpandedBranches(new Set());
            }} className={cn('rounded-full border px-2.5 py-1 text-xs font-medium transition-colors', viewDepth === option.value
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          <Button variant={panMode ? 'primary' : 'secondary'} size="sm" type="button" onClick={() => setPanMode((value) => !value)}>
      ...
      <div className="relative">
        <div ref={viewportRef} className={cn('panel-scroll overflow-auto bg-gradient-to-b from-slate-50 to-slate-100/50 p-4 sm:p-6', canPan && (isPanning ? 'cursor-grabbing' : 'cursor-grab'))} onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={endPan} onPointerLeave={endPan}>
          <div className="mx-auto" style={{ width: scaledWidth, height: scaledHeight }}>
            <div ref={contentRef} className="origin-top-left" style={{
            transform: `scale(${zoom})`,
            width: innerWidth,
        }}>
              <div ref={treeRef} className="flex flex-col items-center pb-4" style={{ width: innerWidth }}>
                <FlowNodeCard node={visibleRoot} sourceNode={root} compact={compact} highlighted={highlightedNodeId === visibleRoot.id} selected={highlightedNodeId === visibleRoot.id} hiddenDescendants={hiddenCounts.get(root.id) ?? 0} onExpandBranch={handleExpandBranch} onSelect={onHighlightNode} cardRef={(element) => registerNodeRef(visibleRoot.id, element)}/>

                {f1Layout.layouts.length > 0 && (<div className="flex w-full flex-col items-center">
                    <TreeConnector width={f1Layout.totalWidth} parentCenterX={f1Layout.totalWidth / 2} childCenterXs={f1Layout.layouts.map((layout) => layout.centerX)}/>
                    <div className="flex items-start" style={{ width: f1Layout.totalWidth }}>
                      {f1Layout.layouts.map((layout, index) => (<div key={layout.node.id} style={{
                    width: layout.width,
                    flexShrink: 0,
                    marginLeft: index > 0 ? columnGap : 0,
                }}>
                          <FlowSubtree node={layout.node} sourceNode={layout.node} compact={compact} highlightedNodeId={highlightedNodeId} hiddenCounts={hiddenCounts} onExpandBranch={handleExpandBranch} onSelect={onHighlightNode} registerNodeRef={registerNodeRef} cardWidth={cardWidth} siblingGap={siblingGap}/>
                        </div>))}
                    </div>
                  </div>)}
              </div>
        ...
        <div className="pointer-events-none absolute bottom-4 right-4 flex flex-col gap-2">
          <div className="pointer-events-auto flex flex-col overflow-hidden rounded-xl border border-border bg-white shadow-lg">
            <button type="button" className="px-3 py-2 text-sm font-medium text-foreground hover:bg-table-stripe" onClick={() => setZoom((value) => Math.min(ZOOM_MAX, Number((value + ZOOM_STEP).toFixed(2))))} aria-label="Phóng to">
              +
            </button>
            <span className="border-y border-border px-3 py-1 text-center text-[10px] font-semibold text-muted">
              {Math.round(zoom * 100)}%
            </span>
            <button type="button" className="px-3 py-2 text-sm font-medium text-foreground hover:bg-table-stripe" onClick={() => setZoom((value) => Math.max(ZOOM_MIN, Number((value - ZOOM_STEP).toFixed(2))))} aria-label="Thu nhỏ">
              −
            </button>
          </div>
          <button type="button" className="pointer-events-auto rounded-xl border border-border bg-white px-3 py-2 text-xs font-semibold text-primary-700 shadow-lg hover:bg-primary-50" onClick={fitToView}>
            Vừa màn hình
          </button>
        </div>
