...
export function TransactionsPanel({ transactions, loading, users, lockedUserId, showUserColumn = !lockedUserId, userLookup, dateRange, onDateRangeChange, }: TransactionsPanelProps) {
    ...
    const filteredTransactions = useMemo(() => {
        ...
        return transactions.filter((transaction) => {
            const matchesUser = activeUserFilter === 'all' || transaction.userId === activeUserFilter;
    ...
    const columns = [
        {
            ...
            header: 'Mã GD',
            className: 'w-32',
            render: (tx: TransactionWithUser) => (<span className="font-semibold text-foreground">{tx.id}</span>),
        },
        ...(showUserColumn
            ? [
                {
                    key: 'customer',
                    header: 'Khách Hàng',
                    render: (tx: TransactionWithUser) => (<div className="flex items-center gap-2">
                <Avatar src={userLookup?.get(tx.userId)?.avatar
                            ?? getAvatarUrl(tx.userId, 'male')} alt={tx.userFullName} size="sm"/>
        ...
        },
        {
            ...
            header: 'Mô Tả',
            className: 'max-w-xs',
            render: (tx: TransactionWithUser) => (<span className="line-clamp-2">{tx.description}</span>),
        },
        {
            key: 'amount',
            header: 'Số Tiền',
            render: (tx: TransactionWithUser) => (<span className="font-semibold text-foreground">
    ...
    return (<>
      <div className="dashboard-card mb-4 flex flex-col gap-3 p-4 lg:flex-row lg:flex-wrap lg:items-end">
        <label className="flex min-w-0 flex-1 flex-col gap-1.5 lg:min-w-[220px]">
          ...
          <input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Mã GD, mô tả, danh mục, khách hàng..." className="form-input"/>
        </label>
        ...
        <label className="flex w-full flex-col gap-1.5 sm:w-40">
          ...
          <input type="date" value={fromDate} min={effectiveDateRange.fromDate} max={effectiveDateRange.toDate} onChange={(event) => updateDateRange(event.target.value, toDate)} className="form-input"/>
        </label>
        ...
        <label className="flex w-full flex-col gap-1.5 sm:w-40">
          ...
          <input type="date" value={toDate} min={effectiveDateRange.fromDate} max={effectiveDateRange.toDate} onChange={(event) => updateDateRange(fromDate, event.target.value)} className="form-input"/>
        </label>

        {!lockedUserId && (<div className="flex w-full flex-col gap-1.5 lg:w-72">
            <span className="text-xs font-semibold uppercase tracking-wide text-muted">
              Khách hàng
            </span>
            <Select value={activeUserFilter} onChange={handleUserFilterChange} options={userOptions} aria-label="Lọc theo khách hàng"/>
            {activeUserFilter !== 'all' && (<p className="text-xs text-primary-600">
                Đang chọn khách hàng — Tổng Quan được kích hoạt trên menu Báo Cáo
              </p>)}
          </div>)}

        <Button variant="secondary" type="button" className="shrink-0" onClick={handleClearFilters}>
          Xóa bộ lọc
        </Button>
      </div>

      {loading ? (<DataTableSkeleton rows={10} cols={showUserColumn ? 8 : 7}/>) : (<DataTable columns={columns} data={filteredTransactions} getRowKey={(tx) => tx.id} onRowClick={(tx) => setSelectedTransaction(tx)} emptyMessage="Không có giao dịch phù hợp." paginationResetKey={paginationResetKey} itemLabel="giao dịch"/>)}

      <TransactionDetailModal transaction={selectedTransaction} user={selectedUser} onClose={() => setSelectedTransaction(null)}/>
    </>);
}
