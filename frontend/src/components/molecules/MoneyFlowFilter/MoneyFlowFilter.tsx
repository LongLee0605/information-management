...
export function MoneyFlowFilter({ defaultValues, accountPlaceholder = 'Nhập số tài khoản F0', values, onChange, onSearch, onReset, error, dirty = false, className, }: MoneyFlowFilterProps) {
    const effectiveRange = getEffectiveAppDateRange();
    const presets = getDatePresets(values.toDate);
    function updateField<K extends keyof MoneyFlowFilterValues>(key: K, value: MoneyFlowFilterValues[K]) {
    ...
    return (<div className={cn('dashboard-card space-y-3 p-4', className)} onKeyDown={handleKeyDown}>
      <div className="flex flex-wrap items-end gap-3">
        <label className="flex min-w-[120px] flex-1 flex-col gap-1.5">
          ...
          <input type="text" value={values.cif} onChange={(event) => updateField('cif', event.target.value.toUpperCase())} placeholder="26410060" className="form-input uppercase"/>
        </label>

        <label className="flex min-w-[160px] flex-1 flex-col gap-1.5">
          <span className="text-xs font-semibold uppercase tracking-wide text-muted">Số tài khoản</span>
          <input type="text" inputMode="numeric" value={values.accountNumber} onChange={(event) => updateField('accountNumber', event.target.value.replace(/\D/g, ''))} placeholder={accountPlaceholder} className="form-input"/>
        </label>
        ...
        <label className="flex w-full flex-col gap-1.5 sm:w-40">
          ...
          <input type="date" value={values.fromDate} min={DEMO_DATE_RANGE.fromDate} max={effectiveRange.toDate} onChange={(event) => updateField('fromDate', event.target.value)} className="form-input"/>
        </label>

        <label className="flex w-full flex-col gap-1.5 sm:w-40">
          <span className="text-xs font-semibold uppercase tracking-wide text-muted">Đến ngày</span>
          <input type="date" value={values.toDate} min={DEMO_DATE_RANGE.fromDate} max={effectiveRange.toDate} onChange={(event) => updateField('toDate', event.target.value)} className="form-input"/>
        </label>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {presets.map((preset) => (<button key={preset.label} type="button" onClick={() => applyPreset(preset.fromDate)} className={cn('filter-chip font-medium', values.fromDate === preset.fromDate
                ? 'border-primary-300 bg-primary-50 text-primary-700'
                : 'border-border bg-white text-muted hover:border-primary-200 hover:text-foreground')}>
            {preset.label}
          </button>))}
        {dirty && (<span className="ml-auto text-xs font-medium text-amber-700">
