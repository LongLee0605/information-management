...
export function MoneyFlowTableView({ root, highlightedNodeId, onSelectNode, className, }: MoneyFlowTableViewProps) {
    ...
    return (<div className={cn('flex flex-col', className)}>
      <div className="panel-header flex flex-wrap items-center gap-2">
        ...
        <label className="ml-auto flex items-center gap-2 text-xs text-muted">
          Sắp xếp
          <select value={sortKey} onChange={(event) => setSortKey(event.target.value as SortKey)} className="form-select w-auto py-1 text-xs">
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[720px] text-left text-sm">
          ...
          <tbody>
            ...
            return (<tr key={node.id} ref={(element) => {
                    if (element) {
                ...
                }} className={cn('cursor-pointer border-t border-border transition-colors', isHighlighted ? 'bg-primary-50' : 'hover:bg-table-stripe/70')} onClick={() => onSelectNode?.(node.id)}>
