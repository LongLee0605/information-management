...
export function ReportDateFilter({ citizenId, fromDate, toDate, onApply, className, }: ReportDateFilterProps) {
    ...
    return (<div className={cn('dashboard-card space-y-3 p-4', className)}>
      <div className="flex flex-wrap items-end gap-3">
        <label className="min-w-[140px] flex-1">
          <span className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-muted">
            Số CCCD
          </span>
          <input readOnly value={citizenId} className="form-input bg-table-stripe"/>
        </label>
        ...
        <label className="w-full sm:w-40">
          ...
          <input type="date" value={draftFrom} min={DEMO_DATE_RANGE.fromDate} max={effectiveRange.toDate} onChange={(event) => setDraftFrom(event.target.value)} className="form-input"/>
        </label>
        ...
        <label className="w-full sm:w-40">
          ...
          <input type="date" value={draftTo} min={DEMO_DATE_RANGE.fromDate} max={effectiveRange.toDate} onChange={(event) => setDraftTo(event.target.value)} className="form-input"/>
        </label>
