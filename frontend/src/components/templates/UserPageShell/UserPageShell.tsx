...
export function UserPageShell({ user, loading, notFound, error, title, subtitle, summary, summaryPeriodLabel = '2025–2026', beforeSummary, toolbarActions, onRetry, children, }: UserPageShellProps) {
    if (loading) {
    ...
    if (notFound) {
        return (<div className="dashboard-card flex flex-col items-center py-16 text-center">
    ...
    if (error || !user) {
        return (<ErrorState message={error ?? 'Không thể tải dữ liệu.'} onRetry={onRetry}/>);
    }
    return (<div className="space-y-6">
      <PageToolbar title={title} subtitle={subtitle ?? `${formatGender(user.gender)} · CCCD ${formatCitizenId(user.citizenId)}`}>
        <Link to={ROUTES.HOME}>
