...
export { buildAvatarSeed, createUniqueAvatarUrl, getAvatarUrl, parseBirthDateFromCitizenId, } from './avatar';
export { getCifFromUserId, getUserIdFromCif, } from './accountRegistry';
export { formatAccountNumberDisplay } from './accountNumber';
export { isDateInRange, APP_DATE_RANGE, } from './demoDate';
export { EMPTY_CUSTOMER_FILTER, filterCustomers, parseVnDateInput, } from './customerFilter';
export type { CustomerFilterValues } from './customerFilter';
export { EMPTY_ACCOUNT_FILTER, ACCOUNT_TYPE_FILTER_OPTIONS, ACCOUNT_STATUS_FILTER_OPTIONS, buildAccountFilterResetKey, filterBankAccounts, } from './accountFilter';
export type { AccountFilterValues } from './accountFilter';
export { formatBirthDate, syncBirthDateFromCitizenId, syncUsersBirthDates, } from './userBirthDate';
export { DEFAULT_PAGE_SIZE } from '@/constants/pagination';
export { canonicalizeUserPathname, getUserRouteSegment, resolveUserIdFromRouteParam, } from './userRoute';
export function enrichColor(hex: string, intensity = 1.25): string {
    ...
    const boost = (channel: number) => Math.min(255, Math.round(channel * intensity));
