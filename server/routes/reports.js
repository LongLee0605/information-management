...
// GET /api/reports/money-flow?customerId=&accountId=&accountNumber=&fromDate=&toDate=&amountThreshold=
router.get('/money-flow', async (req, res) => {
  try {
    const { customerId, accountId, fromDate, toDate, amountThreshold } = req.query;
    const result = await req.pool.request()
      .input('MaKhachHang',  sql.Int,          customerId      ? Number(customerId)      : null)
